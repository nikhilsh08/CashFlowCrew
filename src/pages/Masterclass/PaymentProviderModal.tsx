...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onSelectProvider: (providerId: PaymentProviderId) => void;
  finalPrice: number;
};

export const PaymentProviderModal = ({
  isOpen,
  onClose,
  onSelectProvider,
  finalPrice,
  ...
  // Use global active providers
  const activeProviders: PaymentProvider[] = PAYMENT_PROVIDERS.filter(
    (p) => p.isActive
  );
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {/* Security Note */}
          <div className="mb-6 p-3 bg-green-50 border border-green-200 rounded-xl flex space-x-2">
            <svg
...
export type PaymentProviderId = "Phonepe" | "SabPaisa" | "Zwitch";
...
export const getActiveProviders = (): PaymentProvider[] => {
  return PAYMENT_PROVIDERS.filter(provider => provider.isActive);
};

// Helper function to check if only one provider is active
export const hasOnlyOneActiveProvider = (): boolean => {
  return getActiveProviders().length === 1;
};

// Helper function to get the single active provider
export const getSingleActiveProvider = (): PaymentProvider | null => {
