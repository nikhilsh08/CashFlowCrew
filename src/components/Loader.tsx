...
interface MovingLoaderProps {
  size?: number;
  animationType?: 'pulse' | 'bounce' | 'slide' | 'rotate';
  speed?: number;
  className?: string;
  text?: string;
}
...
const LogoSVG: React.FC<{ size: number }> = ({ size }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 350 221.08002293149147"
...
const MovingLoader: React.FC<MovingLoaderProps> = ({
  ...
  const getAnimationStyle = (): React.CSSProperties => {
    ...
    switch (animationType) {
      ...
      case 'bounce':
        return {
          animation: `bounce ${duration * 1.2}s ease-in-out infinite`
        };
      case 'slide':
        return {
          animation: `slide ${duration * 1.3}s ease-in-out infinite alternate`
        };
      case 'rotate':
        return {
          animation: `rotate ${duration * 0.8}s ease-in-out infinite`
        };
      default:
        return {
          animation: `pulse ${duration}s ease-in-out infinite alternate`
  ...
  return (
    <div className={`flex flex-col justify-center h-[80vh] text-center items-center   ${className}`}>
      <div className=" flex justify-center items-center origin-center" style={getAnimationStyle()}>
        <LogoSVG size={size} />
      </div>
      ...
      `}</style>
      <p className='text-gray-500 font-bold text-2xl text-center absolute mt-[95px] ml-6 '>{`${text || 'Please Wait For The Confirmation...'}`}</p>
    </div>
  );
};
